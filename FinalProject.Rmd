...
final <- plot_grid(points_graph, assist_graph, steal_graph, turn_graph, block_graph, rebound_graph)
```


```{r}
test <- select(allYears, Player, PTS, year)
```


ppg by position (count graph)
```{r}
ggplot() + geom_freqpoly(allYears, aes(x = PTS), binwidth = 0.45) + aes(colour = Pos) + facet_wrap(~Pos, ncol = 4) + labs(title = "") + theme(legend.position = "right") + geom_freqpoly(allTime, aes(x = PTS), binwidth = 0.61)
```
